...
class EasyApplyWizard:
    ...
    def _fill_modal(self):
        ...
        # ── Styled dropdowns / ARIA listboxes / React menus (robust) ─────────────
        combo_triggers = modal.find_elements(
            By.XPATH,
            ".//div[@role='combobox' and @aria-expanded]"
            "|.//div[contains(@class,'dropdown__trigger')]"
            "|.//button[@aria-haspopup='listbox']"
            "|.//input[@aria-haspopup='listbox']"
            "|.//div[contains(@class,'jobs-easy-apply-dropdown')]"
        )

        for opener in combo_triggers:
            try:
                self._scroll_modal()

                # get question text
                label_txt = (opener.get_attribute("aria-label") or opener.text or "").strip().lower()
                ...
                real = [
                    o for o in opts
                    if o.text.strip()
                    and "Select" not in o.text
                    and o.get_attribute("aria-disabled") != "true"
                ]
                if not real:
                    continue
                ...
                for key, pref in _DROPDOWN_DEFAULTS.items():
                    if key in label_txt:
                        for o in real:
                            if o.text.strip().lower() == pref.lower():
                                try: o.click()
                                except: self._js_click(o)
                ...
                # fallback: arrow-down + enter to pick first
                if not picked:
                    opener.send_keys(Keys.ARROW_DOWN)
...
class Linkedin:
    ...
    def _login(self):
        ...
        time.sleep(30)  # allow captcha/2FA if needed
